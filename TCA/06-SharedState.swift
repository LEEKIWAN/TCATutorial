...
struct SharedState: ReducerProtocol {
    ...
    struct State: Equatable {
        ...
        var profile: Profile.State {
            get {
                Profile.State(currentTab: self.currentTab, maxCount: self.counter.maxCount, minCount: self.counter.minCount, numberOfCounts: self.counter.numberOfCounts)
            }
            set {
        ...
        WithViewStore(self.store, observe: \.currentTab) { viewStore in
            VStack {
                ...
                if viewStore.state == .counter {
                    SharedStateCounterView(store: self.store.scope(state: \.counter, action: SharedState.Action.counter))
                }
                
                if viewStore.state == .profile {
                    SharedStateProfileView(store: self.store.scope(state: \.profile, action: SharedState.Action.profile))
                }
...
struct SharedStateCounterView_Previews: PreviewProvider {
